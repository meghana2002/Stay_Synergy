...
#does not need use token and any auth
async def create_superuser(user: users_schema.UserCreate):
    """ Create superuser """
    ...
    query = users_model.user.insert().values(
        email=user.email, name=user.name, hashed_password=f"{hashed_password}",
        is_admin=True
    )
    ...
    return {**user.dict(), "id": user_id, "is_admin": True, "is_active": True, "token": token_dict}
