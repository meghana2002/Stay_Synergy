...
@router.get("/get_token", summary="Get token", tags=["users"])
async def get_token():
    """ Get token for user """

    return users_util.create_access_token(subject="test")


# make a api to make a user admin
@router.post("/make_admin", summary="Make user admin", tags=["users"])
async def make_admin(user: users_schema.UserCreate):
    """ Make user admin """

    return await users_crud.create_superuser(user=user)
